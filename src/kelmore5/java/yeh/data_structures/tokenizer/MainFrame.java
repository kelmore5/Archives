...
import kelmore5.java.yeh.data_structures.tokenizer.temp.Token;

@SuppressWarnings("serial")
public class MainFrame extends JFrame implements Runnable {
	private int x;
	private kelmore5.java.yeh.data_structures.tokenizer.temp.Tokenizer tokenizer;
	private MainPanel panel;

	public MainFrame() {
		super("Tokenizer");
		x = 900;
		tokenizer = new kelmore5.java.yeh.data_structures.tokenizer.temp.Tokenizer("((1+(1+24)+((1+1)**3))**3)+((6*2)**2)");
		panel = new MainPanel();
		...
		change.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(string != "" && string != null) {
					tokenizer = new kelmore5.java.yeh.data_structures.tokenizer.temp.Tokenizer(string);
					remove(panel);
					repaint();
					panel = new MainPanel();
					add(panel);
